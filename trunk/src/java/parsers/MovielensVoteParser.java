...
public final class MovielensVoteParser {
  ...
  public static void main( String[] arg ) {
    try {
      ...
      File u1 = new File("C:/Users/hsb/Documents/NetBeansProjects/Pearson/src/data/u1.test");
      MovielensVoteParser.parse( u1, "\t", 999999999 );
...
//  }
   ...
   *@exception  FileNotFoundException  if the file cannot be found
   */
  public static void parse( File file, String delimiter, int MaxNumber ) throws FileNotFoundException {
    ...
    try {
      ...
      while ( ( ( line = in.readLine() ) != null ) && ( NumberRead < MaxNumber ) ) {
        ...
        String s = r.getmUser() + " " + r.getmItem() + " " + r.getmVote();
   ...
   *@param  delimiter  The delimiter used (/t is always used)
   *@param  r          Description of the Parameter
   */
  public static int parseMovielens( Rating r, String str, String delimiter ) {
    String[] dat = str.split(delimiter);
    if ( dat.length != 4 )
    { 
        System.err.println( "[Error] Parse Error on " + str + "-" + dat.length );
        //System.err.println("Error:" + Integer.parseInt(dat[0]) + "-" + Integer.parseInt(dat[1]) + "-" + Integer.parseInt(dat[2]));
        return -1;
      ...
      try {
          ...
          r.mVote = Double.parseDouble(dat[2]);
